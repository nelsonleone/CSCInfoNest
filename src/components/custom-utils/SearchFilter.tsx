...
interface SearchAndFiltersProps {
  ...
  filters: {
    level?: FilterConfig;
    month?: FilterConfig;
    semester?: FilterConfig;
    [key: string]: FilterConfig | React.ReactNode | undefined;
...
export function SearchAndFilters({
  ...
  // Calculate active filter count dynamically
  ...
  // Get standard filter configs (level, month, semester)
  const standardFilters = Object.entries(filters).filter(
    ([key, value]) => 
    ...
    <div className="mb-8 space-y-4">
      ...
      {showFilters && (
        <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
          <div className="flex flex-col sm:flex-row gap-4">
            {/* Render all standard filters dynamically */}
            {standardFilters.map(([key, filterConfig]) => (
              <div key={key} className="flex-1">
                ...
                >
                  <option value="">All {filterConfig.label || key}s</option>
